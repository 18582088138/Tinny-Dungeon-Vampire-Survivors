...
func _ready():
	base_speed = velocity_componet.max_speed
...
func _physics_process(_delta: float) -> void:
	...
	velocity_componet.accelerate_in_direction(direction)
...
func on_ability_upgrade_added(ability_upgrade: AbilityUpgrade, current_upgrades: Dictionary):
	if ability_upgrade is Ability:	
		...
		velocity_componet.max_speed = base_speed + (base_speed * current_upgrades["player_speed"]["quantity"] * .1)
